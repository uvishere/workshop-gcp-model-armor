...
const knowledgebase = fs.readFileSync(
  path.join(__dirname, 'knowledgebase.txt'),
  'utf8',
);
...
app.get('/health', (req, res) => {
  res.json({ ok: true, project, template: templateName });
});
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (useModelArmor) {
      ...
      if (
        armorResponse.sanitizationResult.filterMatchState === 'MATCH_FOUND' ||
        armorResponse.sanitizationResult.filterMatchState === 2
      ) {
        console.warn('Model Armor BLOCKED the prompt.');
        return res.json({
          blocked: true,
          reason: 'Prompt injection or policy violation detected',
        });
      }
    }
    ...
    if (useModelArmor) {
      ...
      if (
        ...
        return res.json({
          blocked: true,
          reason: 'Response contained sensitive or harmful data',
        });
      }
...
app.listen(port, () => {
  ...
  if (!process.env.GOOGLE_CLOUD_PROJECT) {
    console.warn(
      `⚠️  WARNING: GOOGLE_CLOUD_PROJECT is not set. The app will fail to call Vertex AI. Please check your .env file!`,
    );
  } else {
    console.log(
      `✅ Connected to Google Cloud Project: ${process.env.GOOGLE_CLOUD_PROJECT}`,
    );
  }
});
